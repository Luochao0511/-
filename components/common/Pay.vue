  ...
  <div class="">
    ...
    <div v-if="payStep === 1" class="paymodal codemodal">
      <div class="modal_head">
        ...
        <a class="close iconfont" href="javascript:" @click="close()">&#xe616;</a>
      </div>
    ...
    <div v-else-if="payStep === 0" class="paymodal">
      ...
      <div v-if="courseData" class="modal_body">
        ...
        <div class="remark">
          <label>备注:</label>
          <input v-model="order.remarkCus" placeholder="请输入你需要的备注" type="text">
        </div>
        <div class="pay_type">
          <input id="payType2" v-model="order.payType" name="payType" type="radio" value="2">
          <label class="pay pay_ali" for="payType2" />
          <input id="payType1" v-model="order.payType" name="payType" type="radio" value="1">
          <label class="pay pay_weixin" for="payType1" />
        </div>
      </div>
      <div class="modal_foot">
        <button :disabled="btntext === '正在提交...'" class="solid_btn" @click="submit">{{ btntext }}</button>
      </div>
    </div>
    <div v-else-if="payStep === 2" class="paymodal payrun">
      ...
      <div class="modal_body">
        ...
        <div class="center border_b">
          <a :href="'/course/'+courseData.courseId" class="solid_btn order_btn">立即学习</a>
          <a class="solid_btn order_btn" href="/account/order" target="_blank">查看订单</a>
        </div>
      </div>
    </div>
