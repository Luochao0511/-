<template>
  <div class="h_header">
    <div class="h_nav_fixed">
      <div class="h_nav">
        <div class="h_logo">
          <div class="font_20">教育系统</div>
        </div>
        <ul v-show="navList" class="h_nav_ul clearfix">
          <li v-for="(item, index) in navList" :key="index">
            <a :class="{active: isNow === item.navUrl}" :href="item.navUrl" :target="item.target">{{
              item.navTitle
            }}</a>
          </li>
        </ul>
        ...
        <ul v-show="userInfo" class="top_list">
          ...
          <li class="img_user" @mouseenter="showMenu" @mouseleave="hideMenu">
            <img v-if="userInfo.userHead" :alt="userInfo.mobile" :src="userInfo.userHead">
