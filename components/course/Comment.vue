...
<template>
  <div>
    <div class="comment_box">
      <textarea v-model="commentText" class="comment_input" maxlength="200" placeholder="请输入内容" rows="4" />
      <div class="comment_btn_wrapper">
        <div :class="{no_allow: !commentText}" class="comment_btn" @click="handleComment">评论</div>
      </div>
        ...
        <div v-for="(comment, index) in pageObj.list" :key="index" class="course_comment_item">
          ...
          <div class="course_comment_content">
            ...
            <div class="course_comment_btn">
              <div class="course_comment_time">{{ comment.gmtCreate }}</div>
            </div>
          </div>
        </div>
