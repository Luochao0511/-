<template>
  <div>
    ...
    <div class="container account_cont clearfix">
      ...
      <div class="main_box">
        <ul class="tabs clearfix">
          <a :class="{on: num == 0}" class="tab" @click="clicktab(0)">所有订单</a>
          <a :class="{on: num == 1}" class="tab" @click="clicktab(1)">待支付订单</a>
          <a :class="{on: num == 2}" class="tab" @click="clicktab(2)">已完成订单</a>
        </ul>
          ...
          <div v-for="(item, index) in pageObj.list" :key="index" class="order_content">
            ...
            <div class="order_body clearfix">
              <div class="body_left clearfix">
                <nuxt-link :to="{name: 'course-id', params: {id: item.courseId}}" target="_blank">
                  <div v-if="item.courseLogo" class="img_box fl">
                    <img :alt="item.courseName" :src="item.courseLogo">
                  </div>
                  <p class="fl">
                    {{ item.courseName }}<br><br>
                    原价：<span style="text-decoration-line: line-through; margin-right: 20px">￥{{
                      item.rulingPrice
                    }}</span>实付：￥{{ item.coursePrice }}
                  </p>
                </nuxt-link>
              </div>
              <ul class="body_right clearfix">
                <li>
                  ...
                  <span v-if="item.orderStatus == 2"><nuxt-link
                    :to="{name: 'course-id', params: {id: item.courseId}}"
                    class="go_btn"
                    target="_blank"
                  >马上学习</nuxt-link></span>
                ...
                <li v-if="item.orderStatus == 1 || item.orderStatus == 3">
                  <a class="go_btn go_pay" href="javascript:" @click="continuePay(item)">继续支付</a>
                  <a class="cancel" href="javascript:" @click="closeOrder(item.orderNo)">关闭订单</a>
                </li>
              </ul>
            </div>
          </div>
    ...
    <d-paymodal v-if="showPay" :data="payData" class="" @hidefun="showPay = false" />
    <!-- <bottom /> -->
  </div>
</template>
<script>
