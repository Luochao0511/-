<template>
  <div class="index_page">
    ...
    <y-banner :category-list="categoryData" :data="carouselData" :height="366" />
        ...
        <div class="zone_body">
          <ul class="clearfix course_list">
            <li v-for="(that, int) in item.courseList" :key="int">
              <nuxt-link :to="{ name: 'course-id', params: { id: that.id } }" target="_blank">
                <div class="img_box">
                ...
                <span v-else class="price_box">￥{{ that.coursePrice }} <span
                  style="text-decoration-line: line-through;font-size: 12px"
                >￥{{ that.rulingPrice }}</span></span>
              </nuxt-link>
            </li>
          </ul>
        </div>
