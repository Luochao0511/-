<template>
  <div class="login_page">
    <y-header :hide-search="true" :hide-top="true" />
      ...
      <div class="login_box">
        ...
        <div class="center_box">
          <div v-if="!(userInfo)" :class="{login_form: true, rotate: tab == 2}">
            ...
            <div v-if="tabp == 1" class="form_body">
              <form action="" @submit="loginSubmit">
                <input v-model="loginObj.mobile" placeholder="请输入手机号或邮箱" type="text">
                <div class="error_msg">{{ errTip1 }}</div>
                <input v-model="loginObj.password" placeholder="6-20位密码，可用数字/字母/符号组合" type="password">
                <div class="error_msg">{{ errTip2 }}</div>
                <input v-if="subState" class="btn" disabled="disabled" type="submit" value="登录中···">
                <input v-else class="btn" type="submit" value="登录">
              </form>
              <nuxt-link :to="{name: 'reset'}" class="is_go">忘记密码</nuxt-link>
              <div style="margin-top: 20px;">注册即可体验</div>
            </div>
            <div v-if="tabp == 2" class="form_body r180">
              <form action="" @submit="regSubmit">
                <input v-model="registerObj.mobile" placeholder="请输入手机号" type="text">
                <div class="phone_yzm">
                  <input
                    v-model="registerObj.code"
                    class="phone"
                    maxlength="6"
                    name="code"
                ...
                <input v-model="registerObj.mobilePwd" placeholder="6-20位密码，可用数字/字母/符号组合" type="password">
                <input v-model="registerObj.repassword" placeholder="确认密码" type="password">
                <div class="mgt20 font_14">
                  <input id="tonyi" v-model="registerObj.check" type="checkbox">
                  <label for="tonyi">我已经阅读并同意</label><a class="c_blue" href="jvascript:" @click="xieyi = true">《用户协议》</a>
                </div>
          ...
          <div v-else class="login_form">
            ...
            <div class="login_title is_login">教育系统</div>
            <div class="form_body">
              <div class="img_box">
                <img v-if="userInfo.userHead" :src="userInfo.userHead" alt="">
                <img v-else alt="" src="../assets/image/friend.jpg">
              </div>
              ...
              <div>
                <a class="out_btn" href="javascript:" @click="signOut">退出登录</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="xieyi" class="xieyi" @click.self="xieyi = false">
      <div class="xieyi_content">
        ...
        <div
          v-if="websiteInfo && websiteInfo.websiteUserAgreement"
          class="xieyi_body"
          v-html="websiteInfo.websiteUserAgreement"
        />
        <input class="xieyi_btn" type="button" value="确定" @click="xieyi = false">
      </div>
    </div>
    <!-- <y-bottom /> -->
  </div>
